...
public class PolicyStoreGrpcApi extends PolicyStoreServiceGrpc.PolicyStoreServiceImplBase {
  ...
  public void getDegradationPolicies(
    ...
    List<DegradationPolicy> policies =
        request.getDegradationIdsList().stream()
            .map(id -> DegradationPolicy.newBuilder().setDegradationId(id).build())
            .toList();

    responseObserver.onNext(
        GetDegradationPoliciesResponse.newBuilder().addAllDegradationPolicies(policies).build());
    responseObserver.onCompleted();
  }

  @Override
  public void getDegradationMetadata(
      GetDegradationMetadataRequest request,
      StreamObserver<GetDegradationMetadataResponse> responseObserver) {
    List<DegradationMetadata> metadata =
        request.getDegradationIdsList().stream()
            .map(
                id -> {
                  double criticalThreshold = ThreadLocalRandom.current().nextDouble();
                  double minFallbackRatio = ThreadLocalRandom.current().nextDouble();
                  double maxFallbackRatio =
                      ThreadLocalRandom.current().nextDouble(minFallbackRatio, 1.0);
    ...
    responseObserver.onNext(
        GetDegradationMetadataResponse.newBuilder().addAllDegradationMetadata(metadata).build());
    responseObserver.onCompleted();
  }
}
